...
class RecipeController extends Controller
{
    ...
    public function show(Recipe $recipe)
    {
        ...
        $ingredients = $recipe->ingredients()->with(['alternatives' => function ($query) use ($recipe): void {
        ...
        if (Auth::check()) {
            ...
            $recipeAllergens = $recipe->ingredients()->with(['allergens'])->get()->map(function ($item) {
                if (\count($item->allergens)) {
                    return $item->allergens;
                }
            })->flatten()->filter(fn ($value) => null !== $value);
    ...
    {
        return Cache::remember('recipe_count', 60, fn () => number_format(Recipe::count()));
    }
}
